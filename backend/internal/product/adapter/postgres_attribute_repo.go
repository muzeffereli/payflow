...
func (r *postgresAttributeRepo) SaveBatch(ctx context.Context, attrs []*domain.Attribute) error {
	...
	args := make([]interface{}, 0, len(attrs)*6)
	for i, a := range attrs {
		...
		args = append(args, a.ID, a.ProductID, a.Name, valuesJSON, a.Position, a.CreatedAt)
	}

	query := `INSERT INTO product_attributes (id, product_id, name, values, position, created_at) VALUES ` + strings.Join(valueStrings, ",")
...
func (r *postgresAttributeRepo) ListByProduct(ctx context.Context, productID string) ([]*domain.Attribute, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, product_id, name, values, position, created_at
	...
	for rows.Next() {
		a := &domain.Attribute{}
		var valuesJSON []byte
		if err := rows.Scan(&a.ID, &a.ProductID, &a.Name, &valuesJSON, &a.Position, &a.CreatedAt); err != nil {
