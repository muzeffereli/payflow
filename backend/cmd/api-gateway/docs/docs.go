...
const docTemplate = `{
    ...
    "securityDefinitions": {
        "BearerAuth": {
            "description": "Access token (15 min). Obtain from POST /auth/login, then use as \"Bearer \u003ctoken\u003e\". Refresh via POST /auth/refresh.",
            "type": "apiKey",
            "name": "Authorization",
            "in": "header"
        }
...
var SwaggerInfo = &swag.Spec{
	...
	Description:      "Single entry point for the payment platform. Routes requests to auth, order, payment, and wallet services. See each service's own Swagger UI for full API documentation.",
